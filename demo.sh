...
get_cpu_utilization() {
  ...
  aws cloudwatch get-metric-statistics --region $REGION \
    ...
    --start-time $START_TIME --end-time $END_TIME --period 1800 \
    --statistics Average \
...
for id in "${instance_ids_array[@]}"; do
  CPU_UTILS=$(get_cpu_utilization $id)
  if [ "$CPU_UTILS" != "None" ] && [ ! -z "$CPU_UTILS" ]; then
    echo " "
    echo "Instance ID   :- $id"
    ...
    CPU_UTIL_SUM=0
    CPU_UTIL_COUNT=0
    for util in $CPU_UTILS; do
      CPU_UTIL_SUM=$(echo "$CPU_UTIL_SUM + $util" | bc)
      ((CPU_UTIL_COUNT++))
    done

    if [ $CPU_UTIL_COUNT -gt 0 ]; then
      CPU_UTIL_AVERAGE=$(echo "scale=2; $CPU_UTIL_SUM / $CPU_UTIL_COUNT" | bc)
    else
      CPU_UTIL_AVERAGE=0
    fi

    echo -n "CPU Utilization (Last $DAYS Days): "
    echo "$CPU_UTIL_AVERAGE"

    if awk "BEGIN {exit($CPU_UTIL_AVERAGE >= 0 && $CPU_UTIL_AVERAGE < 20)}"; then
      echo "---- < 20 -------"
      echo " "
      echo "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"
    elif awk "BEGIN {exit($CPU_UTIL_AVERAGE >= 20 && $CPU_UTIL_AVERAGE < 40)}"; then
      echo "---- 20 - 40 -------"
      echo " "
      echo "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"
    elif awk "BEGIN {exit($CPU_UTIL_AVERAGE >= 40 && $CPU_UTIL_AVERAGE < 60)}"; then
      echo "---- 40 - 60 -------"
      echo " "
      echo "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"
    elif awk "BEGIN {exit($CPU_UTIL_AVERAGE >= 60 && $CPU_UTIL_AVERAGE < 80)}"; then
      echo "---- 60 - 80 -------"
      echo " "
      echo "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"
    elif awk "BEGIN {exit($CPU_UTIL_AVERAGE >= 80)}"; then
      echo "---- > 80 -------"
      echo " "
      echo "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"
    fi
